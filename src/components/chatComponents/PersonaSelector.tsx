...
import DoctorConsentModal, { hasDoctorConsent } from "@/components/livetalk/DoctorConsentModal";
...
export default function PersonaSelector({ compact, onChange }: PersonaSelectorProps) {
  ...
  // Doctor-consent gate state. When user clicks Doctor and hasn't consented,
  // we stash the intended pick here and pop the modal.
  const [pendingPersona, setPendingPersona] = useState<BuddyPersona | null>(null);
  ...
  const applyPick = (key: BuddyPersona) => {
  ...
  const handlePick = (key: BuddyPersona) => {
    ...
    const card = PERSONAS[key];
    // Gate Doctor (or any future requiresConsent persona) through the
    // disclaimer modal on first selection.
    if (card?.requiresConsent && !hasDoctorConsent()) {
  ...
  return (
    <>
      ...
      <DoctorConsentModal
        ...
        onAccept={() => {
          if (pendingPersona) applyPick(pendingPersona);
          setPendingPersona(null);
        }}
        onReject={() => setPendingPersona(null)}
      />
    </>
  );
}
